...
이 프로그램은 7채널 마이크로 녹음한 .aup3 파일과 360도 카메라의 오디오를 자동으로 동기화합니다.

### 필수 폴더 구조

프로그램은 다음 **3개 폴더를 같은 위치**에서 찾습니다:

1. **음성 폴더**: JSON 자막 파일 포함 (기준 WAV 파일)
2. **프로젝트 폴더**: .aup3 파일 (7채널 마이크 녹음)
3. **음성_360 폴더**: 360도 카메라 WAV 파일 (동기화 대상)
...
├── 프로젝트/          ← 7채널 .aup3 파일 (필수)
│   └── file_001.aup3
└── 음성_360/          ← 360도 카메라 WAV (필수)
    └── file_001.wav
```

**중요**: 세 폴더가 모두 같은 위치에 있어야 하며, 폴더 이름은 정확히 `음성`, `프로젝트`, `음성_360` 이어야 합니다.

### 동기화 방식

**일반 모드 (aup3 정상):**
1. **프로젝트 폴더**의 .aup3 파일을 7개 채널로 분리
2. **음성 폴더**의 WAV와 비교하여 Trim 범위 감지 및 적용
3. **중앙 마이크 채널**(ch4)과 **음성_360**을 비교하여 오프셋 계산
4. 계산된 오프셋을 **모든 7개 채널**에 동일하게 적용
5. **음성 폴더**의 JSON 자막 파일을 찾아 타임스탬프 조정

**Fallback 모드 (aup3 손상):**
- `.aup3` 파일이 손상되어 파싱 불가능한 경우 자동으로 Fallback
- `음성/` 폴더의 WAV 파일을 사용하여 동기화
- 7채널 동기화는 건너뛰고, `음성_output`만 생성

**지원 파일 형식**:
- `.wav` - 표준 WAV 파일
- `.aup3` - Audacity 3.0+ 프로젝트 파일 (자동 변환 지원)
  - **7채널 지원**: 7채널 .aup3 파일을 7개의 모노 WAV로 자동 분리
  - 각 채널이 동일한 오프셋으로 동기화됨 (file_ch1.wav ~ file_ch7.wav)
...
음성, 프로젝트, 음성_360 폴더가 특정 폴더 안에 있는 경우:

```bash
python audio_sync.py Test
```

이 경우 `Test/프로젝트`, `Test/음성_360` 폴더를 자동으로 찾아서 처리하고, `Test/음성` 폴더에서 JSON 파일을 가져옵니다.

**출력**:
- `Test/프로젝트_output/`: 7채널 동기화된 WAV + JSON
- `Test/음성_output/`: 중앙 마이크(ch4) WAV + JSON

### 2. 기본 사용

현재 디렉토리에 `음성`, `프로젝트`, `음성_360` 폴더가 있는 경우:
...
```bash
python audio_sync.py --reference 프로젝트 --target 음성_360
```

또는 전체 경로 지정:

```bash
python audio_sync.py --reference C:/path/to/프로젝트 --target C:/path/to/음성_360
```

**주의**: 이 경우에도 `음성` 폴더가 프로젝트, 음성_360과 같은 위치에 있어야 JSON 파일을 처리할 수 있습니다.
...
- **고정 패턴 매칭**: `프로젝트` 및 `음성_360` 폴더 쌍 자동 감지
- **음성 폴더 필수**: JSON 자막 파일을 가져오기 위해 `음성` 폴더가 필요
- **배치 처리**: 찾은 모든 폴더 쌍을 순차 처리
- **오류 허용**: 하나의 폴더에서 오류가 발생해도 계속 진행
- **상세 로그**: 모든 처리 과정을 로그 파일에 기록
...
오디오 동기화 배치 처리 (7채널)
======================================================================
시작 시간: 2025-11-20 14:30:22
로그 파일: logs/batch_process_20251120_143022.log
======================================================================
탐색 디렉토리: Test
======================================================================
...
[발견] 3개의 폴더 쌍을 찾았습니다:
  ...
  1. Test\Sample1/
     - 프로젝트: 프로젝트 (3개 .aup3)
     - 음성_360: 음성_360 (3개 WAV)
  2. Test\Sample2/
     - 프로젝트: 프로젝트 (2개 .aup3)
     - 음성_360: 음성_360 (2개 WAV)
  3. Test\Sample3/
     - 프로젝트: 프로젝트 (1개 .aup3)
     - 음성_360: 음성_360 (1개 WAV)
...
- `parent_dir`: 부모 폴더 경로 (선택사항)
  - 예: `python audio_sync.py Test` → `Test/프로젝트`, `Test/음성_360` 처리 (음성 폴더에서 JSON)
- `--reference`: 프로젝트 폴더 경로 (.aup3 파일) - 직접 지정
- `--target`: 음성_360 폴더 경로 (360도 카메라 WAV) - 직접 지정
...
- `--log-file`: 로그 파일 경로 (배치 처리에서 자동 전달됨)

**batch_process.py 옵션**:
- `base_dir`: 탐색할 기본 디렉토리 (기본값: 현재 디렉토리)
- `--no-log`: 로그 파일 생성 안 함 (콘솔 출력만)
- `--only-with-sync`: .sync 파일 쌍이 있는 항목만 처리 (재처리용)
...
### 6. 7채널 .aup3 파일 동기화 상세

7채널 .aup3 파일 처리 과정:

```bash
python audio_sync.py Test  # Test 폴더에 음성, 프로젝트, 음성_360 폴더 있음
```

**처리 과정**:
1. **프로젝트 폴더**에서 .aup3 파일 감지
2. 각 .aup3 파일을 **7개의 모노 WAV**로 자동 분리
   - `file_ch1.wav`, `file_ch2.wav`, ..., `file_ch7.wav`
3. **음성 폴더**의 WAV와 비교하여 Trim 범위 감지 및 적용
4. **중앙 마이크 채널(ch4)**과 **음성_360**을 비교하여 오프셋 계산
5. 계산된 오프셋을 **모든 7개 채널**에 동일하게 적용
6. **음성 폴더**에서 JSON 자막 파일 찾아 타임스탬프 조정
7. 결과를 `프로젝트_output`과 `음성_output`에 저장
...
# 중앙 마이크 채널 번호 (기본값: 4)
CENTER_MIC_CHANNEL = 4

# 출력 폴더 접미사 (기본값: "_output")
...
│   └── ...
├── 프로젝트/                       # 7채널 .aup3 파일
│   ├── VID_20250619_171147_00_001.aup3
│   ├── VID_20250619_174114_00_002.aup3
│   └── ...
└── 음성_360/                       # 360도 카메라 WAV
...
samples/
└── Test/
    ├── 프로젝트_output/            # 7채널 동기화된 WAV + JSON
    │   ├── VID_20250619_171147_00_001_ch1.wav
    │   ├── VID_20250619_171147_00_001_ch2.wav
    │   ├── VID_20250619_171147_00_001_ch3.wav
    │   ├── VID_20250619_171147_00_001_ch4.wav
    │   ├── VID_20250619_171147_00_001_ch5.wav
    │   ├── VID_20250619_171147_00_001_ch6.wav
    │   ├── VID_20250619_171147_00_001_ch7.wav
    │   ├── VID_20250619_171147_00_001_sp.json
    │   └── ...
    └── 음성_output/                # 중앙 마이크(ch4) WAV + JSON
        ├── VID_20250619_171147_00_001.wav
        ├── VID_20250619_171147_00_001_sp.json
        └── ...
```
...
자동 감지가 실패하거나 정확도를 높이고 싶을 때 수동 힌트를 사용할 수 있습니다.
...
# 2단계: 로그 확인 후 실패한 파일에 .sync 쌍 생성
# 프로젝트 폴더: VID_20250708_182027_00_002.sync → 내용: 02.915
# 음성_360 폴더: LRV_20250708_182027_11_002.sync → 내용: 02.045
...
**.sync 파일 형식:**
- 파일명: 원본 파일과 동일한 인덱스 (예: `VID_*_002.sync`, `LRV_*_002.sync`)
- 내용: 박수 소리 위치(초) (예: `02.915`)
- **반드시 양쪽 폴더에 쌍으로 생성**해야 함

**특징:**
- 파일별로 자동 감지 (`*_001.sync`, `*_002.sync` 등)
...
- ✅ **.sync 파일 쌍 지원** (파일별 수동 힌트)
- ✅ **--only-with-sync 옵션** (효율적인 재처리 워크플로우)
- ✅ **aup3 손상 시 Fallback** (음성 폴더 WAV 자동 대체)
